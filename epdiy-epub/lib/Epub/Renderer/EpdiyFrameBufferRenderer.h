  ...
  virtual bool dehydrate()
  {
    ...
    if (compressed)
    {
      ...
      if (fp)
      {
        ...
        ulog_i("EPD", "Buffer saved %d", written);
        return true;
      }
    }
    return false;
    ...
    if (fp)
    {
      ...
      if (compressed_size > 0)
      {
        ...
        if (compressed)
        {
          ...
          {
            ulog_e("EPD", "Failed to decompress front buffer");
          }
          else
          {
            success = true;
        ...
        {
          ulog_e("EPD", "Failed to allocate memory for front buffer");
        }
      }
      else
      {
        ulog_e("EPD", "No data to restore");
      }
      fclose(fp);
    }
    else
    {
      ulog_i("EPD", "No front buffer found");
    }
    return success;
  }
